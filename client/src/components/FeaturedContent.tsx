...
function YouTubeContentItem({ content, index }: { content: any; index: number }) {
    ...
    <Card className="hover-elevate overflow-hidden" data-testid={`card-${content.type}-${index}`}>
      ...
      >
        <div className="aspect-video w-full bg-muted relative overflow-hidden">
          <img
            src={videoDetails?.thumbnailUrl || content.image}
      ...
      <div className="space-y-4">
        ...
        >
          <CarouselContent className="-ml-2 md:-ml-4">
            {baseFeaturedContent.map((content, index) => (
              <CarouselItem key={index} className="pl-2 md:pl-4 md:basis-1/2">
                {content.type === 'youtube' ? (
                  <YouTubeContentItem content={content} index={index} />
                ) : (
                  <Card className="hover-elevate overflow-hidden" data-testid={`card-${content.type}-${index}`}>
                    <a 
                      href={content.url} 
